...
public class CustomerHomeFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        updateExtensionsModelList = new ArrayList<>();
    ...
    private void customermenu() {
        ...
        databaseReference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                updateExtensionsModelList.clear();
                for (DataSnapshot snapshot1 : snapshot.getChildren()){
                    for(DataSnapshot snapshot2 : snapshot1.getChildren()){
                        UpdateExtensionsModel updateExtensionsModel = snapshot2.getValue(UpdateExtensionsModel.class);
                        updateExtensionsModelList.add(updateExtensionsModel);
                    }
                }
                adapter = new CustomerHomeAdapter(getContext(), updateExtensionsModelList);
